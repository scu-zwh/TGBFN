...
class Line(CurveBase):
    ...
    def __repr__(self):
        return "Line: start({}), end({})".format(
            self.start_point.round(4), self.end_point.round(4)
        )
    ...
    def accuracyReport(self,target,tolerance):
        ...
        # For Start Point
        self.line_parameter_correct['s'][0]+=np.abs(self.start_point[0]-
                                                            target.start_point[0])/self.bbox_size
        

        self.line_parameter_correct['s'][1]+=np.abs(self.start_point[1]-
                                                            target.start_point[1])/self.bbox_size
        
        # For End Point
        self.line_parameter_correct['e'][0]+=np.abs(self.end_point[0]-
                                                            target.end_point[0])/self.bbox_size
        

        self.line_parameter_correct['e'][1]+=np.abs(self.end_point[1]-
                                                            target.end_point[1])/self.bbox_size
...
class Arc(CurveBase):
    ...
    def accuracyReport(self,target,tolerance):
        # self.numericalize(n=256) # For Midpoint
        # target.numericalize(n=256)
        if self.bbox_size==0:
            self.bbox_size=1
        ...
        self.arc_parameter_correct={"s":np.array([0,0]),
                                    ...
                                    "e":np.array([0,0])}
        # For Start Point 
        self.arc_parameter_correct['s'][0] += np.abs(self.start_point[0]
                                                -target.start_point[0])/self.bbox_size
        
        
        self.arc_parameter_correct['s'][1] += np.abs(self.start_point[1]
                                                -target.start_point[1])/self.bbox_size
        
        # For Mid Point 
        self.arc_parameter_correct['m'][0] += np.abs(self.mid_point[0]
                                                -target.mid_point[0])/self.bbox_size
        
        self.arc_parameter_correct['m'][1] += np.abs(self.mid_point[1]
                                                -target.mid_point[1])/self.bbox_size
        
        # For End Point 
        self.arc_parameter_correct['e'][0] += np.abs(self.end_point[0]
                                                -target.end_point[0])/self.bbox_size
        
        self.arc_parameter_correct['e'][1] += np.abs(self.end_point[1]
                                                -target.end_point[1])/self.bbox_size
...
class Circle(CurveBase):
    ...
    def accuracyReport(self,target,tolerance):
        ...
        self.circle_parameter_report['c'][0]+=np.abs(self.center[0]-target.center[0])/self.bbox_size
        
        self.circle_parameter_report['c'][1]+=np.abs(self.center[1]-target.center[1])/self.bbox_size
        
        self.circle_parameter_report['r'][0]+=np.abs(self.radius-target.radius)/self.bbox_size
        self.circle_parameter_report['r'][1]+=1
