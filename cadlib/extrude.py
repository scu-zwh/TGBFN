...
class Extrude(object):
    ...
    def from_dict(all_stat, extrude_id, sketch_dim=256):
        """从 JSON 数据构建 Extrude 对象

        参数：
            all_stat (dict): 所有 JSON 数据
            extrude_id (str): 此挤出特征的实体 ID
            sketch_dim (int, optional): 草图标准化大小，默认为 256.

        返回：
            list: 一个或多个 Extrude 实例
        """
        extrude_entity = all_stat["entities"][extrude_id]  # 获取与给定 ID 对应的挤出实体
        assert extrude_entity["start_extent"]["type"] == "ProfilePlaneStartDefinition"  # 确保起始范围类型正确

        all_skets = []  # 初始化一个列表以存储所有草图信息
        n = len(extrude_entity["profiles"])  # 获取与此挤出实体相关的轮廓数量
        for i in range(len(extrude_entity["profiles"])):  # 遍历所有轮廓
            sket_id, profile_id = extrude_entity["profiles"][i]["sketch"], extrude_entity["profiles"][i]["profile"]  # 获取草图和轮廓 ID
            sket_entity = all_stat["entities"][sket_id]  # 从 JSON 提取草图实体
            sket_profile = Profile.from_dict(sket_entity["profiles"][profile_id])  # 从草图实体构造轮廓对象
            sket_plane = CoordSystem.from_dict(sket_entity["transform"])  # 从草图实体获取坐标系统

            # 归一化轮廓
            point = sket_profile.start_point  # 获取轮廓的起始点
            sket_pos = point[0] * sket_plane.x_axis + point[1] * sket_plane.y_axis + sket_plane.origin  # 计算草图的位置
            sket_size = sket_profile.bbox_size  # 获取轮廓的边界框尺寸
            sket_profile.normalize(sketch_dim)  # 对轮廓进行标准化处理
            all_skets.append((sket_profile, sket_plane, sket_pos, sket_size))  # 将处理后的信息添加到列表中

        operation = EXTRUDE_OPERATIONS.index(extrude_entity["operation"])  # 获取挤出操作的类型
        extent_type = EXTENT_TYPE.index(extrude_entity["extent_type"])  # 获取挤出范围的类型
        extent_one = extrude_entity["extent_one"]["distance"]["value"]  # 获取第一部分的挤出距离
        extent_two = 0.0  # 初始化第二部分的挤出距离为 0
        if extrude_entity["extent_type"] == "TwoSidesFeatureExtentType":
            extent_two = extrude_entity["extent_two"]["distance"]["value"]  # 如果是双边挤出，获取第二部分的挤出距离

        # 根据操作类型构建操作列表
        if operation == EXTRUDE_OPERATIONS.index("NewBodyFeatureOperation"):
            all_operations = [operation] + [EXTRUDE_OPERATIONS.index("JoinFeatureOperation")] * (n - 1)  # 新体操作的操作列表
        else:
            all_operations = [operation] * n  # 其他操作的操作列表
