...
class TextInputAdapter(nn.Module):
    ...
    def __init__(
        ...
        self.t_embedding = nn.Linear(1, output_size // 2)
        if num_conditions == 1:
            self.condition1_embedding = nn.Linear(1, output_size // 2)
        elif num_conditions == 2:
            self.condition1_embedding = nn.Linear(1, output_size // 4)
            self.condition2_embedding = nn.Linear(1, output_size // 4)
        else:
            self.t_embedding = nn.Linear(1, output_size)


    def forward(self, probs: torch.Tensor, t: torch.Tensor, conditions: torch.Tensor) -> Tensor:
        # print(probs.dtype)
        ...
        # temp = conditions[:, 0].unsqueeze(-1)
        seq_len = t_emb.shape[1]
        condition1_emb = self.condition1_embedding(conditions[:, 0].unsqueeze(-1)).unsqueeze(1).repeat(1, seq_len, 1)
        t_emb = torch.cat([t_emb, condition1_emb], dim=-1)
        if self.num_conditions == 2:
            condition2_emb = self.condition2_embedding(conditions[:, 1].unsqueeze(-1)).unsqueeze(1).repeat(1, seq_len, 1)
            t_emb = torch.cat([t_emb, condition2_emb], dim=-1)
